...
import zawgyi from '../language/zawgyi';
import mm3 from '../language/mm3';
import en from '../language/en';
import zh_Hans from '../language/zh_Hans';
import {getLanguage} from '../config/AppConfig';
...
//把 zawgyi缅甸文转成 mm3
export function mm3Fromzawgyi(zawgyi) {
  if (isEmpty(zawgyi)) {
    return '';
  }
  let zawgyiToMm3Array = JSON.parse(
  ...
  for (let i = 0; i < zawgyiToMm3Array.length; i++) {
    let obj = zawgyiToMm3Array[i];
    let reg = new RegExp(obj.from, 'g');
    zawgyi = zawgyi.replace(reg, obj.to);
  }

  return zawgyi;
}

//如果需要 转换成mm3, 服务器上面存储的都是mm3,上传的内容都要转，如果当前选中的语言是zawgyi
export function transferToMm3IfNeeded(str) {
  if (isEmpty(str)) {
    return '';
  }
  if (getLanguage() === LANG_ZAWGYI) {
    return mm3Fromzawgyi(str);
  } else {
    return str;
  }
}

//如果需要 转换成zawgyi, 服务器上面存储的都是mm3,上传的内容都要转，如果当前选中的语言是zawgyi
export function transferToZawgyiIfNeeded(str) {
  if (isEmpty(str)) {
    return '';
  }
  if (getLanguage() === LANG_ZAWGYI) {
    return zawgyiFromMm3(str);
  } else {
    return str;
  }
}
